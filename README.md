...
1. Для начала сделаем новое меню, меню options. Для этого копируем объект MainMenu и удалим оттуда все элементы, кроме кнопки Exit. Ее мы переименуем в кнопку Back. Затем сделаем новый объект неактивным. После этого добавим методу OnClick кнопке Options два события. Первое будет делать объект MainMenu неактивным, а второе наоборот делать активным объект Options, все это будет делаться при нажатии на кнопку Options в главном меню. Затем добавим 2 события методу OnClick для кнопки Back объекта Options. Выполняемы события будут зеркальными для событий кнопки Options. В итоге получаем следующий результат: 
